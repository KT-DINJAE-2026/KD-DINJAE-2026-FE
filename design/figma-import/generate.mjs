...
function confirmMap(x, y, width, height, stopName, landmark) {
  return [
    ...
    `<path d="M${x - 12} ${y + 150}L${x + width + 12} ${y + 105}" stroke="#C8CFCB" stroke-width="62"/>`,
    `<path d="M${x - 12} ${y + 150}L${x + width + 12} ${y + 105}" stroke="#FFFFFF" stroke-width="58"/>`,
    `<path d="M${x + 65} ${y - 12}L${x + 118} ${y + height + 12}" stroke="#C8CFCB" stroke-width="49"/>`,
    `<path d="M${x + 65} ${y - 12}L${x + 118} ${y + height + 12}" stroke="#FFFFFF" stroke-width="45"/>`,
    circle(x + 245, y + 96, 29, C.blue, C.surface, 5),
    icon("pin", x + 233, y + 84, 24, C.surface, 2.2),
    rect(x + 26, y + 132, 292, 42, C.surface, 5, C.border),
    text(x + 172, y + 153, stopName, 17, 800, C.text, { anchor: "middle" }),
    rect(x + 120, y + 18, 198, 40, C.surface, 5, C.border),
    text(x + 219, y + 38, landmark, 17, 700, C.text, { anchor: "middle" }),
  ].join("\n");
}
...
const screens = [
  ...
  },
  {
    ...
    svg: screen("정류장 모습이 없을 때 지도 확인 화면", [
      ...
      confirmMap(24, 306, 342, 184, "보문동 주민센터 정류장", "주민센터 맞은편"),
      rect(24, 502, 342, 58, C.amberSoft, 8),
      icon("info", 42, 520, 22, "#9D5F00"),
      multiline(76, 522, ["정류장 사진이 없어", "지도로 안내해요."], 15, 800, "#5F4A18", 1.25),
      text(24, 592, "보문동 주민센터 정류장", 24, 800),
      icon("arrowRight", 26, 610, 20, C.blue),
      text(58, 620, "가는 방향", 16, 700, C.subtext),
      text(170, 620, "신설동 방면", 16, 800),
      icon("pin", 26, 646, 20, C.blue),
      text(58, 656, "가까운 곳", 16, 700, C.subtext),
      text(170, 656, "주민센터 맞은편", 16, 800),
      icon("walk", 26, 682, 20, C.blue),
      text(58, 692, "내린 다음", 16, 700, C.subtext),
      text(170, 692, "보문역까지 도보 5분", 16, 800),
      rect(24, 716, 342, 42, C.blueSoft, 8),
      icon("bus", 42, 727, 20, C.blue),
      text(76, 737, "성북구청에서 바로 가는 버스 2개", 16, 700, C.subtext),
      primaryButton(772, "이 정류장으로 가기"),
    ].join("\n")),
  },
  {
